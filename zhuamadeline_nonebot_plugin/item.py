                    ...
                    if(current_time < next_time_r or current_time < work_end_time_r):
                        ...
                        if time_since_last_sleep < datetime.timedelta(hours=4):
                            ...
                            await daoju.finish(
                                ...
                                f"{remaining_time.seconds % 60}秒内不要使用时间秒表了……", at_sender=True)
            ...
            #--------------------这些道具需要限制所在猎场的使用--------------------、
                ...
                if time_since_last_sleep < datetime.timedelta(hours=4):
                    ...
                    await daoju.finish(
                        f"好好休息吧，{remaining_time.seconds // 3600}小时"
                        f"{(remaining_time.seconds % 3600) // 60}分钟"
                        f"{remaining_time.seconds % 60}秒内不要试图使用抓捕类道具了……", at_sender=True)
                ...
                # 在商品列表里面就直接回答暂无法使用
                await daoju.finish(f"[{use_item_name}]在抓玛德琳里面不存在，或者使用方法有误，又或者现在无法使用哦！", at_sender=True)
        else:
