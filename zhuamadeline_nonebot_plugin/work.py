...
async def work_handle(event: GroupMessageEvent, bot: Bot, arg: Message = CommandArg()):
    ...
    if user_info['working']:
        ...
        if current_time < working_endtime:
            ...
            await work.finish(
                f"你已经派遣madeline出去工作了，耐心等待吧。他们大概会在{text}后完成工作",
                at_sender=True
            )
        else:
...
async def status_work_handle(bot: Bot, Bot_event: GroupMessageEvent):
    ...
    if data[str(user_id)]['working']:
        ...
        if current_time < working_endtime:
            ...
            await status_work.finish(f"你已经派遣madeline出去工作了，耐心等待吧。他们大概会在{text}后完成工作，工作完成后他自然会将工作日志汇报给您的哦\n"+
...
async def work_help_handle(bot: Bot, Bot_event: GroupMessageEvent):
    msg = (
        "以下是关于工作的一些指令：\n\n"
        ".work 工作区域/携带食物/派遣madeline的名称 | 工作的主命令，派遣madeline出去工作，不过注意派出去的madeline不会回来哦！\n\n"
        ".workspeed | 可以消耗体力来加速工作完成，但是随着每天加速次数的增加，消耗的体力越多\n\n"
        ".workjd | 查询Madeline工作的进度，工作完成后用这个命令来收取草莓/道具\n\n"
        ".worksleep | 休息4h，休息的这4h能恢复200体力（若有房产证藏品则是600），但是这个时间内不能抓，不过能和加工器同时使用\n\n"
        ".workhelp | 可以查询工作帮助，也就是本界面\n"
        "---------------------\n"
        "关于工作的一些帮助\n"+
