...
# 自定义背景色命令
set_bgcolor = on_command('set_bgcolor', aliases={'设置背景色', '自定义背景色'}, permission=GROUP, priority=1, block=True, rule=whitelist_rule)
@set_bgcolor.handle()
async def set_bgcolor_handle(bot: Bot, event: GroupMessageEvent, arg: Message = CommandArg()):
    ...
    # 初始化用户数据
    if user_id not in data:
        data[user_id] = {'berry': 1000, 'event': 'nothing'}
    elif 'berry' not in data[user_id]:
        data[user_id]['berry'] = 1000
    
    # 检查是否有正在进行的事件
    if data[user_id].get('event', 'nothing') != 'nothing':
        await send_image_or_text(user_id, set_bgcolor, "你还有正在进行的事件未完成", True, None)
        return
    
    # 解析参数
    color_arg = str(arg).strip()
