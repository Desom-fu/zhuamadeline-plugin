...
collections = {
    ...
    [
        ...
        9,
        2,
        '仅需2500草莓，你就能购买草莓果园里的一块地！你可以种草莓，施肥，甚至可以偷别人的草莓！收获的草莓会放入仓库哦！\n\n'
        '现在草莓果园可用指令：.garden 收菜(take)/施肥(fert)/偷菜(steal)/播种(seed)/升级(upgrade)\n\n'
        '- 收菜：可以将草莓果园里面的草莓全部收到仓库里！\n'
    ...
    '房产证':[
        ...
        9,
        2,
        '你买下了房产证之后，你就可以在房子里面休息了，而不是在野外风餐露宿。\n休息时可恢复150点/h（4h总共600点）的体力'
    ],
    '时隙沙漏': [
        100000,
        8, 
        5, 
        '这是抓玛德琳世界中的十大秘宝之一。\n一个由神秘的蓝色水晶制成的神秘沙漏，但是却散发着淡金色光芒。\n沙漏的沙粒中闪烁着时间的碎片，当沙漏翻转时，持有者能暂时打破时间的桎梏——那些未被使用的等待时间将化作"时之砂"储存起来。\n这个沙漏似乎能将你未使用的等待时间储存为抓取机会，并且在使用这些抓取机会的同时，部分事件好像会被时隙沙漏过滤掉……\n不过……似乎这种力量无法于竞技场中使用。' #每29/30分钟(受回想之核影响)自动积累1次抓取机会，最多可储存5次机会，使用时不会触发冷却时间
    ],
    '逆十字架': [
        100000,
        8, 
        3, 
        '倒悬的黑色十字架，表面布满血色纹路。传说这是堕天使路西法的信物，能压制对手的力量。\n持有此收藏品时，在madeline竞技场对战中，作为进攻方会小幅削弱对手战斗力的下限。' # 对手战力下限-3
    ],
    '暴击指虎': [
        100000,
        8, 
        3, 
        '价格昂贵的金属指虎，关节处镶嵌着幽绿色的宝石。当蓄力到临界点时，宝石会迸发刺目的绿光。\n在讨伐Boss时，有微量概率触发"暴怒一击"，使本次造成的伤害翻倍！' # 10%概率伤害翻倍
    ],
    '淡紫色狐狸毛': [
        100000,
        8, 
        3, 
        '与其他狐狸毛不同的是，这一撮狐狸毛异常的稳定……或许还能够防爆？\n持有此收藏品时，使用madeline提取器和充能陷阱时将有额外中量的概率使你免除本次爆炸伤害。'# 5%概率
    ],
    '幸运戒指': [
        100000,
        8, 
        3, 
        '镶嵌着四叶草形状翡翠的银戒指，叶脉中流淌着液态的幸运。当你遭遇厄运时，它会悄悄拨动命运的齿轮……仔细观察，上面还刻着“寰宇新未来”几个烫金大字？\n进行预言大师/抽卡且结果亏损时，有中量概率触发"命运重置"，立即重新随机生成新的结果！' # 10%概率重roll
    ],
    '宝藏号角': [
        100000,
        8, 
        4, 
        '用象牙制成的白色号角，吹响时会洒落星辉般的能量结晶。每次竞技场胜利后，有可能获得来自古代宝藏的能量馈赠！\n竞技场获胜时有少量概率额外获得2×藏品等级的能量或体力奖励。' # 10%概率获得2*等级能量/体力
    ],
    '入场券': [
        100000,
        8, 
        5, 
        '用亮蓝色水晶制成的金属门票，如同宝石般闪耀。这是进入遗忘深渊的通行证，票面上的淡紫色文字会随着时间变换排列。\n'
        '你捕捉到玛德琳后，它会根据玛德琳的等级吸收对应的能量，帮你适应这无尽深渊里的压迫。\n'
        '不过里面……似乎有一点金色的光芒，在闪烁？\n'
        '进入第五猎场（遗忘深渊）的必备物品。'
    ],
}